...
def Maintrans():
    ...
    path=(glob.glob("*.ab1"))
    f = open("phagemidsequencingProteinout1.txt", "a")
...
#print dna
          sub1="CAGGTGCAG"#CAGGTGCAG
          record=dna
          sub2="ACCGTCTCCT"
      
          mm=start_stop_codon(dna,sub1)
          nn=start_stop_codon(dna,sub2)
     # print mm,nn+len(sub2)
          Re=(nn+len(sub2)- mm) % 3
      #print Re
          if Re == 0:
         
             p = translate(dna[mm:nn+len(sub2)])
          else:
             p=""
          if p!="":
             f.write(">")
